System.register(['./Player', './GameService'], function(exports_1, context_1) {
    ...
    return {
		...
		`;
				...
				<span>${this.gameService.getPlayerShape(Player_1.PlayerType.Human).name}</span>
				${winner.name} wins!
				<span>${this.gameService.getPlayerShape(Player_1.PlayerType.Computer).name}</span>
			</div>`;
                        if (winner.winningStreak == 3) {
                            popupEl.className += 'show';
                            if (winner.type == Player_1.PlayerType.Human) {
                                popupEl.innerHTML = `
						<div class="message">
							<h2>You have won!</h2>
